const request = prompt("Enter server address:port", "ws://127.0.0.1:443");
...
class Client {
    constructor(url) {
        if (!url) {
            throw new Error("You need to specify a url for the client to connect to.");
        };
        this.ws = new WebSocket(url === null ? "ws://127.0.0.1:443" : url);
    ...
    start() {
        ...
        document.body.onmousewheel = this.handleWheel;
        ...
        setInterval(this.updateMouse, 1);
        this.resizeCanvas();
        $("#overlays").fadeIn(6E2);
    };
            ...
            switch (event.keyCode) {
                ...
                case 13:
                    ...
                    if (textBox.value.length === 0) {
                        ...
                        var buffer = this.prepareData(2 + 2 * textBox.value.length);
                        var offset = 0;
    ...
    onmessage(data) {
        ...
        // Handle message
        const msg = new DataView(msg.data);
    ...
    updateMouse() {
            const buffer = this.prepareData(21);
    ...
    sendNick(nick) {
        if (this.ws.readyState == ws.OPEN && null != nick) {
            const buffer = this.prepareData(1 + 2 * nick.length);
            buffer.setUint8(0, 0);
            for (var i = 0; i < nick.length; ++i) buffer.setUint16(1 + 2 * i, nick.charCodeAt(i), true);
            send(buffer)
        };
    };
...
window.client = new Client(request)